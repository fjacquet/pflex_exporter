...
func TestExpandFallback(t *testing.T) {
	unsetForTest(t, "PFLEX_FALLBACK_TEST_UNSET")
	t.Setenv("PFLEX_FALLBACK_TEST_SET", "real")
	t.Setenv("PFLEX_FALLBACK_TEST_EMPTY", "")
	for _, tc := range []struct{ name, in, want string }{
		{"unset falls back", "${PFLEX_FALLBACK_TEST_UNSET:-false}", "false"},
		{"set wins over default", "${PFLEX_FALLBACK_TEST_SET:-false}", "real"},
		{"exported empty falls back", "${PFLEX_FALLBACK_TEST_EMPTY:-other}", "other"},
		{"empty default allowed", "${PFLEX_FALLBACK_TEST_UNSET:-}", ""},
		{"mixed with literal text", "a${PFLEX_FALLBACK_TEST_UNSET:-b}c", "abc"},
	} {
	...
	if _, err := ExpandEnv("${PFLEX_FALLBACK_TEST_UNSET}"); err == nil {
...
func TestExpandSecretRejectsEmpty(t *testing.T) {
	t.Setenv("PFLEX_EMPTY_SECRET_TEST", "")

	if _, err := ExpandEnv("${PFLEX_EMPTY_SECRET_TEST}"); err != nil {
		t.Fatalf("plain expansion must stay lenient on an exported-empty variable: %v", err)
	}
	if _, err := ExpandEnvSecret("password", "${PFLEX_EMPTY_SECRET_TEST}"); err == nil {
