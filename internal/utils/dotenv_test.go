...
func TestLoadDotEnvFeedsInterpolation(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(dir, ".env"),
		[]byte("PFLEX_DOTENV_GW=flex.example.com\nPFLEX_DOTENV_USER=mon\nPFLEX_DOTENV_PW=s3cret\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(cfgPath, []byte(`
clusters:
  - name: c1
    gateway: "${PFLEX_DOTENV_GW}"
    username: "${PFLEX_DOTENV_USER}"
    password: "${PFLEX_DOTENV_PW}"
`), 0o600); err != nil {
		t.Fatal(err)
	}
	for _, v := range []string{"PFLEX_DOTENV_GW", "PFLEX_DOTENV_USER", "PFLEX_DOTENV_PW"} {
		t.Setenv(v, "")
		_ = os.Unsetenv(v)
	}
	...
	cfg := &models.Config{Clusters: []models.ClusterConfig{
		{Name: "c1", Gateway: "${PFLEX_DOTENV_GW}", Username: "${PFLEX_DOTENV_USER}", Password: "${PFLEX_DOTENV_PW}"},
	}}
	if err := ResolveSecrets(cfg); err != nil {
		t.Fatal(err)
	}
	cl := cfg.Clusters[0]
	if cl.Gateway != "flex.example.com" || cl.Username != "mon" || cl.Password != "s3cret" {
