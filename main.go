...
import (
	...
	"github.com/fjacquet/pflex_exporter/internal/telemetry"
	"github.com/fjacquet/pflex_exporter/internal/utils"
...
type Server struct {
	...
	store    *powerflex.SnapshotStore

	telemetry      *telemetry.Manager
	tracerProvider trace.TracerProvider

	mu            sync.Mutex // guards clients/collector/collectCancel across reloads
	clients       []powerflex.Client
	collector     *powerflex.Collector
...
func (s *Server) Start() error {
	...
	if err := s.registry.Register(powerflex.NewPromCollector(s.store)); err != nil {
		return fmt.Errorf("failed to register collector: %w", err)
	}

	if err := s.registry.Register(powerflex.NewBuildInfoCollector(version, runtime.Version())); err != nil {
...
func (s *Server) initOTLP(cfg *models.Config) error {
	...
	exp, err := powerflex.NewOTLPExporter(ctx, cfg.OpenTelemetry.Metrics, s.store, version)
	if err != nil {
		return err
	}
...
func buildClients(cfg *models.Config, tp trace.TracerProvider) []powerflex.Client {
	...
	for _, cl := range cfg.Clusters {
		var opts []powerflex.ClientOption
		if tp != nil {
