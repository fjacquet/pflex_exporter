...
**Goal:** Add a `HEALTHCHECK` to `Dockerfile.goreleaser` — the file that builds pflex_exporter's actually-published image — matching the `/livez` check already added to the local `./Dockerfile` and both compose files on 2026-08-01.
...
- `HEALTHCHECK` targets `http://127.0.0.1:9445/livez`, never `localhost` — Alpine's busybox `wget` resolves `localhost` via `::1` first, and the exporter only binds IPv4.
...
| File | Responsibility |
| --- | --- |
| `Dockerfile.goreleaser` | Adds `HEALTHCHECK` before `USER pflex` |
...
Insert before `USER pflex`:

```dockerfile
EXPOSE 9445

HEALTHCHECK --interval=30s --timeout=5s --start-period=10s --retries=3 \
  CMD wget --quiet --tries=1 --spider http://127.0.0.1:9445/livez || exit 1

USER pflex
```

(`EXPOSE 9445` already exists — only the `HEALTHCHECK` block is new.)
...
```bash
CGO_ENABLED=0 go build -o pflex_exporter .
mkdir -p linux/amd64 && cp pflex_exporter linux/amd64/pflex_exporter
docker build -f Dockerfile.goreleaser --build-arg TARGETPLATFORM=linux/amd64 -t pflex_exporter:healthcheck-test .
docker run -d --name pflex-hc-test -p 19445:9445 \
  -e PFLEX1_GATEWAY=flex-clu1-gw01 -e PFLEX1_USERNAME=flex-username -e PFLEX1_PASSWORD=dummy \
  pflex_exporter:healthcheck-test
sleep 15
docker inspect --format='{{.State.Health.Status}}' pflex-hc-test
```
...
```bash
docker rm -f pflex-hc-test
docker rmi pflex_exporter:healthcheck-test
rm -rf linux pflex_exporter
```
...
so it can carry a Docker `HEALTHCHECK`; distroless cannot. pflex_exporter's local
