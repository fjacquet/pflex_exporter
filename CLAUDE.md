...
- **Gen1:** `GET /api/instances` + `POST /api/instances/querySelectedStatistics`. The embedded `querySelectedStatistics.json` is split into one query **per object type**, issued concurrently (`gen1QueryConcurrency`, errgroup-bounded) and merged via `Statistics.Merge` — so a single rejected stat name degrades only that type instead of failing the whole cluster's Gen1 collection (ADR 0002). Both generations now share the per-type-fan-out shape. `deriveSamples` (`derivations.go`) computes iops/bandwidth/io-size/latency from `Bwc` counters.
